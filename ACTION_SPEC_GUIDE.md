...
> 팁: 페이지를 연 상태에서 실행해야 DeepSeek가 그 페이지의 실제 HTML(단순화 요약, 최대 600,000자)을
> 보고 셀렉터를 만듭니다. 빈 검색 페이지보다 **결과가 보이는 상태**에서 돌리면 정확도가 올라갑니다.
> 계획 생성 시 익스텐션이 **먼저 페이지 끝까지 자동 스크롤**해 지연 로딩(lazy-load) 항목까지 띄운 뒤
> HTML을 수집하므로, 처음 화면에 안 보이던 항목도 셀렉터 추론에 반영됩니다.
> 실제로 LLM에게 가는 이 HTML은 팝업의 **"현재 페이지 HTML 보기"** 버튼으로 그대로 확인할 수 있습니다.
...
- **가격이 틀리게 나옴**(원가가 잡힘 / 숫자만·"원"만 / 값이 합쳐짐) → 판매가 셀렉터가 애매한 경우.
  취소선 원가(`<del>`/`<s>`, `base`/`정가` 클래스)를 피하고 **판매가** 요소를 직접 지정.
  숫자와 단위가 다른 태그로 쪼개져 있으면 둘을 감싼 **부모 요소**를 `price`로 쓰면 됨
  (단, 여러 가격을 한꺼번에 감싸는 너무 넓은 부모는 피함). 작업 문구에 "취소선 원가가 아닌
  실제 판매가를 가져올 것"을 명시하면 DeepSeek가 더 정확한 셀렉터를 고름.
